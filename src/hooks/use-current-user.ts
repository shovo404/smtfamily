...
export type AppRole = "super_admin" | "admin" | "hr" | "dhr" | "so" | "fi";
export const ALL_ROLES: AppRole[] = ["super_admin", "admin", "hr", "dhr", "so", "fi"];

export const CREATABLE_ROLES: AppRole[] = ["admin", "hr", "dhr", "so", "fi"];
export const SUPER_ADMIN_CREATABLE_ROLES: AppRole[] = ["super_admin", ...CREATABLE_ROLES];
...
const DEFAULT_PERMISSIONS: Record<AppRole, PermissionKey[]> = {
  ...
  so: ["viewTasks", "manageTasks", "viewLiveTracking"],
  fi: ["viewTasks", "manageTasks", "viewLiveTracking"],
};
...
export function useCurrentUser() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const role = (profile.role ?? "fi") as AppRole;
